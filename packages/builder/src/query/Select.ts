...
import { AppendObjects, AggregateType, JoinType, Selects, Sources, Name, SourceInstance, OrderDirection, PartialChildren, AppendTuples, MergeObjects, SelectsKeys, Simplify, ArrayToTuple, LockType, SourceInstanceFromSelects, SelectWithKey } from '../Types';
...
export class QuerySelect<T extends Sources, S extends Selects> extends QuerySelectBase<T, S, S[]>
{
  ...
  // WITH RECURSIVE source_name 
  //  query where selects are used as source fields
  //  union [all|distinct]
  //  query which has access to source_name
  // 

  public with<WN extends Name, WS extends Selects>(query: ExprProvider<T, S, SourceQuery<WN, WS>>): QuerySelect<Simplify<AppendObjects<T, Record<WN, SourceInstanceFromSelects<WS>>>>, S> {
  ...
  // public withRecursive<B extends SourceQuery<any, any>, R extends SourceQuery
  ...
  public aggregate<AT extends AggregateType>(type: AT, value?: Expr<any>, distinct: boolean = false): QuerySelectFirstValue<{}, [Select<AT, number>], Select<AT, number>> {
    return new QuerySelectFirstValue(this, new ExprAggregate(type, distinct, value).as(type)) as any;
  }

  public count(distinct: boolean = false, value?: Expr<any>) {
    return this.aggregate('COUNT', value, distinct);
  }

  public countIf(condition: Expr<boolean>) {
    return this.aggregate('COUNT', this._exprs.inspect().when<1 | null>(condition, 1).else(null), false);
  }

  public sum(value: Expr<number>) {
    return this.aggregate('SUM', value);
  }

  public avg(value: Expr<number>) {
    return this.aggregate('AVG', value);
  }

  public min(value: Expr<number>) {
    return this.aggregate('MIN', value);
  }

  public max(value: Expr<number>) {
